...
image::17-result-other.png[,50%, align=left]
...
[NOTE]
We are bypassing the ingestion (_Data Acquisition_) phase described to speed up the process of producing a second version of the model. Later you will participate in generating your own training data to produce a third version of the model.
...
image::21-monitor-admin-button.jpg[,40%]

After you click, you'll see in the monitoring view a series of animations illustrating the actions taking place in the platform. The following enumeration describes the process:
...
. The pipeline (_Tekton_) reads from the S3 storage system all the training data available and executes the Data Science notebooks based on _TensorFlow_
...
All the steps above described are well illustrated in the diagram below:
